...
const LoginForm = () => {
  ...
  return (
    ...
    >
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-100 border border-red-200 rounded-xl text-red-600 text-xs text-center font-bold">
          {error}
        </div>
      )}
      ...
      <div className="mt-8 flex items-center justify-between text-xs font-bold">
        ...
        <span onClick={() => navigate("/forgot-password")} className="text-primary hover:underline cursor-pointer">Forgot?</span>
      </div>
    </div>
  );
};
