  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-4 sm:gap-6 relative z-50">
        ...
        {/* Navigation Links */}
        <ul
          className={`hidden lg:flex items-center gap-7 text-[13px] font-bold tracking-[0.05em] ${isScrolled ? "text-white" : "text-white"
            }`}
        >
          {navItems.map((item) => (
            <li
        ...
        <div className="flex-grow max-w-[180px] hidden md:block">
          <div
            className={`relative flex items-center rounded-full px-4 py-1.5 border transition-all duration-500 ${isScrolled
          ...
          >
            ...
            />
            <svg
              className={`w-4 h-4 transition-colors ${isScrolled ? "text-white/40" : "text-white/60"
                }`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
        ...
        <div className="flex items-center gap-2">
          <button className="bg-white text-black text-[9px] sm:text-[11px] font-extrabold px-3 sm:px-6 py-2 sm:py-2.5 rounded-full flex items-center gap-2 sm:gap-3 hover:bg-primary hover:text-white transition-all transform hover:scale-105 shadow-xl hover:shadow-primary/10 active:scale-95 group">
      ...
      {/* Mobile Menu Drawer */}
      <div
        className={`fixed inset-0 bg-[#212121]/98 backdrop-blur-2xl z-[40] transition-all duration-500 lg:hidden ${isMenuOpen ? "opacity-100 translate-y-0 visible" : "opacity-0 -translate-y-full invisible"
          }`}
      >
        <div className="flex flex-col h-full pt-32 sm:pt-40 px-6 pb-10">
          <ul className="flex flex-col gap-4 text-lg font-bold text-white">
            {navItems.map((item) => (
              <li
                key={item.name}
                className="flex items-center justify-between border-b border-white/10 pb-3 cursor-pointer hover:text-primary transition-colors"
